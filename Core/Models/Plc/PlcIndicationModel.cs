namespace Core.Models.Plc
{
    public class PlcIndicationModel
    {
        ...
        public Parameter<short> NakopitelStatus { get; } = new Parameter<short>(nameof(NakopitelStatus), "Статус накопителя", 0, 100, 11, 0) { IsOnlyRead = true, RegType = Registers.Input};
        #endregion

        #region Статус накопителя НП10
        public Parameter<short> NakopitelStatus10 { get; } = new Parameter<short>(nameof(NakopitelStatus), "Статус накопителя", 0, 100, 16, 0) { IsOnlyRead = true, RegType = Registers.Input };
        #endregion
        ...
        public Parameter<bool> ProbotborReady1 { get; } = new Parameter<bool>(nameof(ProbotborReady1), "Готовность к отбору", false, true, 0, 0) { RegType = Registers.Input, IsOnlyRead = true };
        #endregion
        #region Первичный проботборник занят
        public Parameter<bool> ProbotborBusy1 { get; } = new Parameter<bool>(nameof(ProbotborBusy1), "Занят", false, true, 0, 1) { RegType = Registers.Input, IsOnlyRead = true };
        #endregion

        #region current_time — текущее время цикла отбора

        public Parameter<short> ProbCurrentTime { get; } = new Parameter<short>(nameof(ProbCurrentTime), "Текущее время цикла отбора", 1, 1000, 2, 0) { RegType = Registers.Input, IsOnlyRead = true };
        ...
        public Parameter<bool> GlobalError { get; } = new Parameter<bool>(nameof(GlobalError), "Наличие ошибок", false, true, 23, 0) {RegType = Registers.Input, IsOnlyRead = true };
        #endregion
        #region Статус первичного проботборника
        public Parameter<short> ProbotborStatus1 { get; } = new Parameter<short>(nameof(ProbotborStatus1), "Статус пробоотборника", 0, 100, 3, 0) {RegType = Registers.Input, IsOnlyRead = true };
        #endregion
